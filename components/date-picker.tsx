...
export default function DatePickerInput ({ onChange, defaultValue }: DatePickerInputProps) {
  const now = today(getLocalTimeZone());
  let date = null;
  if (defaultValue) {
    date = parseDate(defaultValue.substring(0, 10));
  }
  return (
    <DatePicker onChange={onChange} className='space-y-2' defaultValue={date}>
      <div className='flex'>
              ...
              <CalendarGridBody className='[&_td]:px-0'>
                {(date) => (
                  <CalendarCell
                    date={date}
                    className={cn(
                      'relative flex size-9 items-center justify-center whitespace-nowrap rounded-lg border border-transparent p-0 text-sm font-normal text-foreground outline-offset-2 transition-colors data-[disabled]:pointer-events-none data-[unavailable]:pointer-events-none data-[focus-visible]:z-10 data-[hovered]:bg-accent data-[selected]:bg-muted data-[hovered]:text-foreground data-[selected]:text-primary-foreground data-[selected]:outline-none data-[unavailable]:line-through data-[disabled]:opacity-30 data-[unavailable]:opacity-30 data-[focus-visible]:outline data-[focus-visible]:outline-2 data-[focus-visible]:outline-ring/70 data-[invalid]:data-[selected]:[&:not([data-hover])]:bg-destructive data-[invalid]:data-[selected]:[&:not([data-hover])]:text-destructive-foreground',
                      date.compare(now) === 0 &&
